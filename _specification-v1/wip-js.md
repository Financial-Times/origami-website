...
- **Should not** add to the global scope.
- **Should not** read or modify the DOM on parse.
* **Should not** modify the DOM outside of areas of [owned DOM](#owned-dom), except:
...
* **Should not** require global variables to be defined prior to the script loading. If your module requires configuration, read the config from data attributes attached to parts of DOM that your module will own (see [data attributes](#data-attributes-on-owned-dom))
* **Should not** assume the existence of globals except those defined as part of ECMAScript 5 and features listed in the `browserFeatures/required` section of `origami.json`.
...
<aside>
	...
	<ul>
		<li>declare it as a requirement in your Origami manifest `origami.json`; or</li>
...
If the `init` method (or the components constructor) takes an element argument to identifying its [owned DOM](#owned-dom), it **must** accept the following types:
...
If the given element does not have the components [data attribute](#data-attributes-on-owned-dom) `data-o-component`, the `init` function **may** traverse the subtree looking for elements which are.

The `init` function **may** create multiple instances and return them in an array. Where passed to a constructor, the component must only create one instance and return it.
